 ...
 *
 * Important Note: remember to read the notes about each example, especially the 'Important Note' section.
 *
 * @author udoprog
 */
public class WhyAsync {
    ...
    /**
     * The thread pool that you use in your application has the same number of threads as the number of cores on your
     * system. The async approach will make better use of them by the virtue of only waking up threads when necessary
     * (i.e. an interesting event happens).
     *
     * As a bonus, the synchronous approach will (most likely) deadlock if we change NUMBER_OF_REQUESTS above, to the
     * same as AVAILABLE_PROCESSORS, because the loop that waits for the work to finish will occupy all threads. The
     * async approach will happily work on a single thread if necessary.
     *
     * So how many threads would the synchronous count require? At least AVAILABLE_PROCESSORS + NUMBER_OF_REQUESTS,
     * since currently NUMBER_OF_REQUESTS threads will be occupied waiting for results.
     *
     * Another solution could be to use multiple, distinct thread pools for each logical component, that will increase
     * the number of threads, pools, and contention.
     *
     * Important Note: summing below is a contrived example, a better approach would arguably be to pass around an
     * AtomicDouble. Consider these computations that uses more complex types of objects instead (like rows from a
        ...
        private Future<Double> someSyncCall() {
            return syncThreads.submit(new Callable<Double>() {
                ...
                public Double call() throws Exception {
                    ...
                    // this is equivalent to all threads blocking for the above result to finish for some reason.
