 ...
 * released. Fortunately, there are some helpers to ease this like {@code #doto(FutureFramework,
...
public interface Managed<T> {
   ...
   */
  String TRACING = "eu.toolchain.async.Managed.trace";

  /**
   * System property that if set to 'yes', will cause stacks to be captured by borrowed
   * references.
   */
  String CAPTURE_STACK = "eu.toolchain.async.Manabed.captureStack";
   ...
   */
  <R> CompletionStage<R> doto(final ManagedAction<T, R> action);
